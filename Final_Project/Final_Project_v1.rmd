...
Finally, we loaded the ratings dataset to Spark and built a recommender system using the `ml_als` function available in `sparklyr`, which builds collaborative filtering models using Alternating Least Squares (ALS).  
  
We then used that model to make predictions.  
...
spark_mdl <- ml_als(partitions$training, rating ~ user_id + anime_id, rating_col = "rating", user_col = "user_id", item_col = "anime_id", cold_start_strategy = "drop")
# predictions
...
# # split datasets
# eval_sets <- evaluationScheme(data = rating_mtx, method = "cross-validation", k = 4, given = 1, goodRating = 6)
